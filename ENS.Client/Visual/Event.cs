        ...
        private async void Entbtn_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                var test = new Test4
                {
                    ...
                    Contacts = contactsBox.Text.Split(new[] { Environment.NewLine }, StringSplitOptions.RemoveEmptyEntries).ToList(),
                    Subject = themeBox.Text,
                    Text = mailBox.Text,
                    Type = (EventType)Enum.Parse(typeof(EventType), typeBox.Text)
                };
